...
function Today({data,isDemo,syncing,token,onSelect}:{data:AthleteData;isDemo:boolean;syncing:boolean;token:string;onSelect:(a:Workout)=>void}){
 ...
 <CorosDaily data={data} now={now}/><div className="home-training-grid"><DashboardCalendar activities={data.activities} now={now} token={token} isDemo={isDemo} syncKey={data.syncedAt} onSelect={onSelect}/>
