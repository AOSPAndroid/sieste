...
export function trainingAssessment(data:AthleteData,now:Date){
 ...
 const missing=signals.some(s=>['No reading','Older reading','Learning usual range','Limited coverage'].includes(s.label));
 ...
 const loadText=change!==null?`Last 7 days including today so far: ${change>=0?'+':''}${Math.round(change)}% effort vs the preceding 28-day daily average.`:base===0?'No established load baseline yet.':`Load available for ${rows.filter(r=>r.effort!==null).length}/35 days. Missing: ${rows.filter(r=>r.effort===null).slice(-5).map(r=>r.iso.slice(5)).join(', ')}${rows.filter(r=>r.effort===null).length>5?'…':''}. Today is partial.`;
