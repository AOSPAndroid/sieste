...
export default function SleepVitals({data,end,days,date}:{data:AthleteData;end:string;days:number;date:string}){
 ...
 const detail=(k:Key)=><><p>{date} · {format(selected?.[k]??null,k)} {k==='hours'?'':defs[k].unit}</p>{chart(k,true)}{comparisons(k).map(c=><p key={c.n}><b>{change(c.delta,k)}</b> vs {c.n===1?'yesterday':`prior ${c.n}-day average`} · {c.count}/{c.n} readings{c.mean===null?' · insufficient history':''}</p>)}<p>{assessments[k].reason}</p><p>Tredict readings where available, otherwise COROS, matched by date. Resting HR is a daily reading, not an overnight heart-rate trace. Shading uses the selected date’s normal range; gaps are missing readings.</p></>;
