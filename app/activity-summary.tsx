...
export default function ActivitySummary({detail,loading,history=[],onClassify}:{onClassify?:(label:string|null)=>void;detail:Record<string,any>;loading:boolean;history?:any[]}){
 ...
 const {cards,other,family}=activityMetrics(detail),step=detail.seriesSampled?.sampleSize,comparison=sessionTrend(detail,history),vo2=vo2Trend(detail,history);
 ...
 {detail.seriesSampled?.data&&<ActivityProfile key={detail.id} detail={detail} onSelection={setRouteSelection} onShowMap={()=>mapRef.current?.scrollIntoView({block:'start',behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}/>}
 ...
 {!loading&&<SessionIntelligence key={detail.id} detail={detail} onClassify={onClassify}/>}
