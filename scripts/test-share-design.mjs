...
const tiles=[];
for(const [i,template] of ['editorial','route','split','signature','serif','laps','bib','strip','outline','receipt','chrono'].entries()){
...
await sharp({create:{width:980,height:2980,channels:3,background:'#dfe5ec'}}).composite(tiles).png().toFile(new URL('contact-sheet.png',dir).pathname.replace(/^\/(\w:)/,'$1'));
console.log('Passed: eleven designs at three sizes, transparent pixel alpha, opaque toggle, real rendered content, XML escaping. Rendered contact sheet for review.');
